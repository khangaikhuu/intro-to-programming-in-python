...
"""
famous_person = "suess"
print(f'Dr. {famous_person.title()} once said , "You know you’re in love when you can’t fall asleep because reality is finally better than your dreams."')
...
msg = "{} {}".format(famous_person , "Hello, How are you?")
print(f"{msg}")
...
person = [" khatnaa ", "khangai ", " Idree"]
print(f"{person[0].strip().title()} \n\t{person[1].rstrip().title()} \n\t\t{person[2].lstrip().title()}")
...
Энэ бүлэг болон дараагийн хэсэгт та жагсаалт гэж юу болох, 
жагсаалтад байгаа элементүүдтэй хэрхэн ажиллаж эхлэх талаар 
мэдэх болно. Жагсаалтууд нь та цөөн хэдэн сая эсвэл хэдэн 
сая зүйлтэй эсэхээс үл хамааран олон тооны мэдээллийг нэг 
дор хадгалах боломжийг олгодог. Жагсаалт нь Python-н шинэ 
хүчирхэг шинж чанаруудын нэг бөгөөд програмчлал дахь олон 
чухал ойлголтуудыг нэгтгэдэг.
"""
...
"""
friends =["tegshee","tsogoo","idree"]
print(f"{friends[0].title()} {friends[-2].title()} {friends[-1].title()}")
...
brand = ['toyota','bmw','porche','hyundai']
brand_cars = ['land 200', 'X5', 'sonato7']
print(f"I would like to own a {brand[1].title()} {brand_cars[-2].title()}.") 
...
invite_list = ['Gerelmaa','Undraa','Alihander']
print(f"Would you go to dinner with {invite_list[0].title()}?")
...
person = invite_list.pop(-1)
print(f"{person.title()}")

invite_list.append("Uyanga")
print(f"Would you go to dinner with {invite_list[-1].title()}?")

for aa in invite_list:
    print(f"Would you go to dinner with {aa.title()}?")
...
"""
for aa in invite_list:
    print(f"Dear {aa.title()} \n\tWe are now able to add more orders. Please serve us \n Sincerely, Khatnaa")

invite_list.insert(0, 'munkhgerel')
print(invite_list)
invite_list.insert(-3, 'khava')
print(invite_list)
invite_list.append('sarangoo')
print(invite_list)
...
for person in invite_list:
    cnt -= 1
    if cnt > 2:
        sorry = invite_list.pop(0)
        print(f"Dear {sorry.title()} \n\tSorry you can’t invite them to dinner. \n Sincerely, Khatnaa")
    else
        pass
print(invite_list)
